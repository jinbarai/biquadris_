...
Block* levelfive::makeBlocks(char type, bool isHeavy) {
    Block *p; 
    switch(type) {
    case 'I': 
...
bool levelfive::is_file_exist(const string file) {
    std::ifstream infile(file);  
    return infile.good();
}

void levelfive::blocksFromFile(string filename) {
    if (this->norandom) { 
...
void levelfive::setRandom(bool val) {
    this->norandom = val; 
}

bool levelfive::getRandom() {
